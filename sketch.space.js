...
let userTextEl = null;
let userTextWrapEl = null;

function setTextBoxVisible(isTextMode){
  userTextWrapEl = userTextWrapEl || document.getElementById("textInputWrap");
  if (!userTextWrapEl) return;

  if (isTextMode) {
    userTextWrapEl.classList.add("is-open");
    userTextWrapEl.setAttribute("aria-hidden", "false");
  } else {
    userTextWrapEl.classList.remove("is-open");
    userTextWrapEl.setAttribute("aria-hidden", "true");
  }
}

function initUserTextBox(){
  userTextEl = document.getElementById("userTextInput");
  userTextWrapEl = document.getElementById("textInputWrap");
  if (!userTextEl || !userTextWrapEl) return;

  // inizializza con il testo attuale (di default: Lorem ipsum)
  userTextEl.value = "";

  userTextEl.addEventListener("input", () => {
    textContent = (userTextEl.value || "").trim() ? userTextEl.value : "";
    layoutDirty = true;

    // se sei in text mode, ricalcola subito il layout (anche senza mano)
    if (spaceMode === "text") {
      updateTextField("none", 0, false, null);
    }
  });

  // stato iniziale (particles)
  setTextBoxVisible(spaceMode === "text");
}
